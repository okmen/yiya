...
public class BaseOrderMgtServiceImpl implements IBaseOrderMgtService {
	...
	// @Autowired
	// private OPayforuserorderMapper payForOrderMapper;//支付单-订单 关系 暂时不用此表，
	@Autowired
	...
	// // 获取产品信息
	// PProducts products =
	// productsMapper.selectByPrimaryKey(pp.getProductid());
	...
	// public ReturnModel submitOrder(Long userId, Long addrId, String remark,
	// List<OOrderproducts> prolist) {
	// ReturnModel rq = new ReturnModel();
	// try {
	// Map<String, UserOrderParam> orderlist = getUserOrderParamList(addrId,
	// remark, prolist);
