	...
	private static String getPackage(List<NameValuePair> params) {
	 ...
	 public static List<NameValuePair> bean2Parameters(Object bean)
	 {
	        ...
	        for (Method method : Methods)
	        {
	            if (method != null && method.getName().startsWith("get")&& !method.getName().startsWith("getClass"))
	            {
	                ...
	                try
	                {
	                    ...
	                    {
	                        int val = 0;
	                        try{ val = (Integer) method.invoke(bean);} catch (InvocationTargetException e){}
	 	...
	 	public static String sortParam(Object order)
	    {
	 	 ...
	 	 * @return
	 	 */
	 	public static Map<String, String> getOrderTitle(int typeState,String orderno)
	 	{
	 		Map<String, String> map=new HashMap<String, String>();
	 		switch (typeState) {
			case 13:
				map.put("order", "支付单号："+orderno);
				map.put("product", "供应商进驻保证金");
				break;
			case 14:
				map.put("order", "支付单号："+orderno);
				map.put("product", "认证服务点资料费");
				break;
			case 16:
				map.put("order", "支付单号："+orderno);
				map.put("product", "认证服务点资料费+保证金");
				break;
			default:
				map.put("order", "订单："+orderno);
				map.put("product", "订单："+orderno);
				break;
			}
	 		return map;
	 	}
